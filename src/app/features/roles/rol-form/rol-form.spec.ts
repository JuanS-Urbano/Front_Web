...
describe('RolForm', () => {
  ...
  const mockRolService = {
    ...
    crearRol: vi.fn().mockReturnValue(of({ data: {}, success: true })),
    updateRol: vi.fn().mockReturnValue(of({ data: {}, success: true }))
  };
  const mockSession = {
    session$: of(null),
    getEmpresaId: vi.fn().mockReturnValue(1),
    getEmpresaNombre: vi.fn().mockReturnValue('TestEmpresa'),
