...
export const LAYOUT_ROUTES: Routes = [
  {
    ...
    children: [
      ...
      { path: 'usuarios', canActivate: [roleGuard], data: { roles: ['ADMIN_EMPRESA', 'EDITOR'] }, loadChildren: () => import('../usuarios/usuarios.routes').then(m => m.USUARIOS_ROUTES) },
      { path: 'procesos', canActivate: [roleGuard], data: { roles: ['ADMIN_EMPRESA', 'EDITOR'] }, loadChildren: () => import('../procesos/procesos.routes').then(m => m.PROCESOS_ROUTES) },
      { path: 'editor/:procesoId', loadChildren: () => import('../editor/editor.routes').then(m => m.EDITOR_ROUTES) },
      { path: 'roles', canActivate: [roleGuard], data: { roles: ['ADMIN_EMPRESA', 'EDITOR'] }, loadChildren: () => import('../roles/roles.routes').then(m => m.ROLES_ROUTES) },
